...
/// ```
pub fn rawReader(self: *UnpackReader, reader: anytype, header: fmt.Header) !Raw(@TypeOf(reader)) {
    switch (header.type) {
    ...
    return Raw(@TypeOf(reader)).init(prefix, reader, header.size);
}
...
pub fn array(self: *UnpackReader, header: fmt.Header) !Array {
    if (header.type == .array or header.type == .fixarray)
        return Array.init(self, header.size);
    return ConvertError.InvalidValue;
}

pub fn map(self: *UnpackReader, header: fmt.Header) !Map {
    if (header.type == .map or header.type == .fixmap)
        return Map.init(self, header.size);
    return ConvertError.InvalidValue;
}
...
///
/// Use `Array.next` to grab the header and
/// use the `Array.reader` to read the value.
///
/// Example:
///
...
pub const Array = struct {
    ...
    pub fn init(reader: *UnpackReader, len: u32) Array {
        return .{
            .reader = reader,
            .len = len,
        };
    ...
    pub fn next(self: *Array, reader: anytype) !?fmt.Header {
    ...
    pub fn skipAll(self: *Array, reader: anytype) !u32 {
...
///
/// This type works almost like the `Array`. You can use
/// `Map.is_value` to check the current value is either
...
pub const Map = struct {
    ...
    pub fn init(reader: *UnpackReader, len: u32) Map {
        return .{
            .reader = reader,
            .len = len,
        };
    ...
    /// Return `null` if the array is ended, `error.EndOfStream` if
    /// the reader can no longer read.
    pub fn next(self: *Map, reader: anytype) !?fmt.Header {
        if (self.current >= self.len) {
            return null;
        }
    ...
    pub fn skipAll(self: *Map, reader: anytype) !u32 {
        const c0 = self.current;
